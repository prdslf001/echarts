<template>
    <div class="com-container">
        <div class="title" :style="fontTitleStyle">
            ...
            >
                <path
                    d="M520.4992 797.696l467.456-459.776c7.2704-7.168 7.3728-18.8416 0.2048-26.0096-7.168-7.2704-18.8416-7.3728-26.0096-0.2048L507.6992 758.6816 54.8864 305.8688c-7.168-7.168-18.8416-7.168-26.0096 0-3.584 3.584-5.4272 8.2944-5.4272 13.0048 0 4.7104 1.8432 9.4208 5.4272 13.0048L494.592 797.5936C501.6576 804.7616 513.2288 804.7616 520.4992 797.696z"
                    fill="#ffffff"
                    p-id="4237"
                ></path>
            </svg>
            <ul v-show="showList">
                <li
                    v-for="(items, index) in listDataType"
                    :key="index"
                    v-show="showList"
                    @click="changeSlected(items.key)"
                >
                    {{ items.text }}
                </li>
            </ul>
...
let fontTitle = ref(10);
//计算属性
const listDataType = computed(() =>
    dataResult.value?.type.filter((items) => items.text != currentSelected.value),
);
...
const adapterChange = () => {
    fontTitle.value = (echartsRef.value.offsetWidth / 70) * 3.6;
    const adapterOption = {
        legend: {
